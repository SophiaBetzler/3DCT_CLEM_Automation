...
def main(img_path, ss_in, ss_out, qtprocessbar=None, interpolationmethod='linear', saveorigstack=True, showgraph=False, customSaveDir=None):
	...
	if os.path.isfile(img_path) is True:
		...
		if img_int is not None:
			...
			if px_info is True:
				tf.imwrite(file_out_int, img_int, metadata={'PixelSize': str(pixelsize),'FocusStepSize': str(ss_out/1000)})
			else:
				tf.imwrite(file_out_int, img_int)
			if debug is True: print(clrmsg.DEBUG, "		...done.")
		if qtprocessbar:
	...
	## For image sequence (only FEI MAPS/LA image sequences at the moment)
		...
		for i in range(channels):
			...
			if saveorigstack is True:
				...
				if px_info is True:
					tf.imwrite(file_out_orig, img, metadata={'PixelSize': str(pixelsize),'FocusStepSize': str(pixelsizeZ)})
				else:
					tf.imwrite(file_out_orig, img)
				if debug is True: print(clrmsg.DEBUG, "		...done.")
				if qtprocessbar:
					qtprocessbar.setValue(qtprocessbar.value()+int(20/channels))
					QtGui.QGuiApplication.processEvents()
...
def norm_img(img,copy=False,qtprocessbar=None):
	...
	if img.ndim == 2:
		...
		img = (img.astype('float64') * (typesize / img.max())).astype('int16')
	## 3D or multichannel image
		...
		if img.shape[-1] > 4:
			...
			if qtprocessbar:
				...
				qtprocessbar.setValue(int(maximum * 0.1))
				QtGui.QGuiApplication.processEvents()
			for i in range(img.shape[0]):
				slice_max = img[i, :, :].max()
				if slice_max == 0:
					continue  # or: img[i, :, :] = np.zeros_like(img[i, :, :])
				scale_factor = typesize / slice_max
				img[i, :, :] = (img[i, :, :].astype('float64') * scale_factor).astype('int16')
				if qtprocessbar:
...
def normalize(path,qtprocessbar=None, customSaveDir=None):
	...
	if len(img.shape) == 4:
		tf.imwrite(fname_norm, img, imagej=True)
	else:
		tf.imwrite(fname_norm, img)
	if debug is True: print(clrmsg.DEBUG, "		...done")
...
def mip(path,qtprocessbar=None, customSaveDir=None, normalize=False):
	...
	if len(img.shape) == 4:
		...
		if normalize:
			...
			img = norm_img(img)
		if debug is True: print(clrmsg.DEBUG, "Saving...")
		tf.imwrite(fname_mip_norm if normalize else fname_mip, img, imagej=True)
		if debug is True: print(clrmsg.DEBUG, "		...done")
		...
		if normalize:
			...
			img = norm_img(img)
		if debug is True: print(clrmsg.DEBUG, "Saving...")
		tf.imwrite(fname_mip_norm if normalize else fname_mip, img)
		if debug is True: print(clrmsg.DEBUG, "		...done")
		...
		for filename in files:
			if filename.endswith('.tif'):
				...
				if len(img.shape) == 4:
					...
					tf.imwrite(fname_norm, img_MIP, imagej=True)
				elif len(img.shape) == 3:
